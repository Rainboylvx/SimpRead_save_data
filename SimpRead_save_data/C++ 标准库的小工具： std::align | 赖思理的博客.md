> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [lesleylai.info](https://lesleylai.info/zh/std-align/)
...
如果你仍然想要之前不需要显式提供对齐要求的 `alloc` 成员函数， 我们可以在其中调用 `aligned_alloc`并把对齐要求设置为`std::max_align_t`（一个平台上所以类型最大可能的对齐要求）：
